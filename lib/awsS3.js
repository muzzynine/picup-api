...
var checkExistNodeObjectsBatch = function(nodeArray){
	...
	_.forEach(nodeArray, function(node){
	    //s3Path가 'default'인 경우 앨범 프로필이 지정되지 않은 경우에 해당하므로 s3에 사진이 존재하는지 확인할 필요가 없다. presence가 delete일 경우 또한 s3에 사진이 있는지는 상관없으므로 확인할 필요가 없다.
