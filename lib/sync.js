    ...
    try{
        (function getDeltaList(_src, _dst) {
            ...
            if (isBoundary(_src, _dst)) {
                ...
                var forwardSrc = getForwardRevision(_src, _dst);
                return getDeltaList(forwardSrc, _dst);
            } else {
                resultDelta.backward.push(_src);
                var backwardSrc = getSkipDeltaNumber(_src);
                return getDeltaList(backwardSrc, _dst);
            }
        })(src, dst);
    } catch (err){
	if(err.isAppError){
        ...
        throw AppError.throwAppError(409, "compute error");
    }
